...
export async function createOnsiteOrder(payload: OnsiteOrderPayload): Promise<Order> {
  const response = await api.post<ApiResponse<Order>>("/admin/orders/onsite", payload);
  return response.data.data;
}
